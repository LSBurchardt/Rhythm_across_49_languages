...
rhythm_results_doreco_ipu <- read_delim("language_data/results/rhythm_analysis_mean_doreco_split_quantile99_fs_20.csv", delim = ",")
...
rhythm_results_doreco_ipu_meta_summarized_file <- rhythm_results_doreco_ipu_meta %>%
...
rhythm_results_doreco_ipu_ordered_summarized_file <- rhythm_results_doreco_ipu_ordered %>%
  filter(!is.na(ioi_beat)) %>%
  group_by(Family, file) %>%
  summarize(across(where(is.numeric), mean, na.rm = TRUE), 
            across(where(is.character), first), 
...
# both datasets are also summarized per Speaker, to avoid pseudo-replication

rhythm_results_doreco_ipu_meta_summarized_speaker <- rhythm_results_doreco_ipu_meta %>%
  filter(!is.na(ioi_beat)) %>%
  group_by(Family, speaker) %>%
  summarize(across(where(is.numeric), mean, na.rm = TRUE), 
            across(where(is.character), first), 
            across(where(is.factor), first))


rhythm_results_doreco_ipu_ordered_summarized_speaker <- rhythm_results_doreco_ipu_ordered %>%
  filter(!is.na(ioi_beat)) %>%
  group_by(Family, speaker) %>%
  summarize(across(where(is.numeric), mean, na.rm = TRUE), 
            across(where(is.character), first), 
...
rhythm_results_doreco_ipu_ordered_summarized_file %>%
  dplyr::filter(is.na(ioi_beat) == FALSE) %>% 
  group_by(speaker, Language) %>% 
  ggplot(aes(y = ioi_beat, x = Language, fill = Family))+
  geom_boxplot()+
...
rhythm_results_doreco_ipu_meta_summarized_file %>%
  drop_na(speaker_sex) %>%
  group_by(speaker) %>% 
